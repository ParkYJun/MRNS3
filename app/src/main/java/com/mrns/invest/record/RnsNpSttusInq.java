 ...
 */
@Table
public class RnsNpSttusInq extends SugarRecord {
    ...
    public long getNpsiSn() {
        return NpsiSn;
    }

    public void setNpsiSn(long npsiSn) {
        NpsiSn = npsiSn;
    }
    ...
    public static RnsNpSttusInq FindByNpuSn(long NpuSn) {
        List<RnsNpSttusInq> list = RnsNpSttusInq.find(RnsNpSttusInq.class, "NPU_SN=?", String.valueOf(NpuSn));
        if(list.size() > 0)
            return list.get(0);
        else
            return null;
    ...
    public static RnsNpSttusInq findLastInq(long npuSn) {
        List<RnsNpSttusInq> list = RnsNpSttusInq.find(RnsNpSttusInq.class, "NPU_SN=?", String.valueOf(npuSn));
        if(list.size() > 0) {
            RnsNpSttusInq result = list.get(0);
            long npsiSn;

            for (RnsNpSttusInq tempInq : list) {
                npsiSn = result.getNpsiSn();
                if (tempInq.getNpsiSn() > npsiSn) {
                    result = tempInq;
                }
            }
            return result;
