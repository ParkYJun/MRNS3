...
public class InvestInsertImages extends AppCompatActivity {
    //! Extras
    ...
    protected void InitView() {
        ...
        Log.d("imageWidth", imageviewImage.getWidth() + "");
        ...
        if (mItem != null) {
            ...
            switch (exIndexInsert) {
                case INDEX_K_IMG:
                    ...
                    mImageName = getString(R.string.fn_kimage, mItem.getBasePointNo(), mDate);
                    break;
                case INDEX_O_IMG:
                    strGetImage = mItem.getOImg();
                    mImageName = getString(R.string.fn_oimage, mItem.getBasePointNo(), mDate);
                    break;
                case INDEX_P_IMG:
                    strGetImage = mItem.getMsurpointsLcDrw();
                    mImageName = getString(R.string.fn_pimage, mItem.getBasePointNo(), mDate);
                    break;
                case INDEX_ROUGHMAP:
                    mBIsRoughmap = true;
                    ...
                    mImageName = getString(R.string.fn_rogimage, mItem.getBasePointNo(), mDate);
                    break;
            }
    ...
    protected void loadingDialog(boolean view, String message) {
        if (view)
            FineDialog.showProgress(InvestInsertImages.this, false, message);
        else
    ...
    private void showErrorAlert() {
        ...
        AlertDialog.Builder alertDialogBuilder = new AlertDialog.Builder(InvestInsertImages.this);
        alertDialogBuilder
...
//        Uri uri = Uri.fromFile(new File(mImageFullName));
        ...
        Uri uri = FileProvider.getUriForFile(this, getApplicationContext().getPackageName() + ".fileprovider", new File(mImageFullName));
    ...
    protected void onMakeRoughmap() {
        if (exBOnline) {
            Intent intent  = new Intent(getApplicationContext(), MapCaptureActivity_.class);
            intent.putExtra(getString(R.string.key_type), getString(R.string.key_point_type_for_map_query));
            intent.putExtra(getString(R.string.key_fid), mItem.getBasePointNo());
            intent.putExtra(getString(R.string.key_stdr_spot_sn), mItem.getStdrspotSn());
            intent.putExtra(getString(R.string.key_date), mDate);

            startActivityForResult(intent, PICK_MAKE_ROUGHMAP);
        } else {
    ...
    protected void onDetail() {
        if (onSave(getString(R.string.code_invest_insert_ongoing))) {
            Intent intent = InvestInsertImagesDetail_.intent(InvestInsertImages.this).get();
    ...
    protected void onProcessBtns(View view) {
        ...
        //  startActivity(mInsertUtil.getMoveIntent(view.getId()));
    }
    ...
    public void onNext() {
        ...
        switch (exIndexInsert) {
            ...
            case INDEX_O_IMG:
                ...
                startActivity(mInsertUtil.getMoveIntent(R.id.imgbtn_pimg));
                break;
            case INDEX_P_IMG:
                if (!onSave(getString(R.string.code_invest_insert_ongoing))) {
                    mInsertUtil.setmBHasBitmap(mBHasBitmap);
    ...
    public void onBack() {
        ...
        switch (exIndexInsert) {
            ...
            case INDEX_O_IMG:
                ...
                startActivity(mInsertUtil.getMoveIntent(R.id.imgbtn_kimg));
                break;
            case INDEX_P_IMG:
                if (!onSave(getString(R.string.code_invest_insert_ongoing))) {
                    mInsertUtil.setmBHasBitmap(mBHasBitmap);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == 0 && requestCode == 0) {
            AlertDialog.Builder builder = new AlertDialog.Builder(InvestInsertImages.this);
    ...
    private void pickFromAlbum(Intent data) {
        try {
            ...
            float rate = 0.0f;
            int viewWidth = imageviewImage.getWidth();
            int maxResolution = viewWidth;
