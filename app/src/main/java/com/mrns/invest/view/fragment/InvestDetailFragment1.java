...
public class InvestDetailFragment1 extends Fragment {
    ...
    @AfterViews
    void initView() {
        long stdrspotSn = getArguments().getLong(getString(R.string.key_stdr_spot_sn));
        mIsGeo = getArguments().getBoolean(getString(R.string.key_is_geo));

        if(mIsGeo) {
            mUrl = getString(R.string.server_ip) + getString(R.string.url_viewpoint);
            requestViewUnityPOST(stdrspotSn);
            setImages(stdrspotSn);
        } else {
    ...
    @UiThread
    void setImages(long stdrspotSn) {
        String serverIp = getString(R.string.server_ip);
        int urlResoureceIds[] = {R.string.url_detail_k_img, R.string.url_detail_o_img, R.string.url_detail_msurpoints_lc_drw, R.string.url_detail_rog_map};
    ...
    void setImages(RnsStdrspotUnity item) {
        ...
        for (int i = 0; i < imageViewList.size(); i++) {
            ...
            if (null == bitmap || null == imageList[i] || imageList[i].isEmpty() || imageList[i].equals(getString(R.string.str_null))) {
                ...
                try {
                    ...
                    //mRogmapAttacher = new PhotoViewAttacher(scRogmap);//약도이미지 확대/축소 라이브러리 등록
                } catch (IOException e) {
                    Log.e("", "이미지 없음");
                    return;
                } finally {
    ...
    void setData(RnsStdrspotUnity item) {
        if(item != null) {
            ...
            textviewClassification.setText(StringUtil.convertClassification(StringUtil.nullCheck(item.getInqClsf())));
            ...
            textviewLocationDetail.setText(StringUtil.nullCheck(item.getLocplcDetail()));
