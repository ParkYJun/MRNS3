...
public class InvestInsertInfo extends AppCompatActivity {
    //! Extras
    ...
    //! 멤버변수
    private RnsSttusInqBefore mItemBf;
    ...
    protected void InitView() {
        ...
        mItemBf = RnsSttusInqBefore.FindByStdrspotSn(exStdrspotSn);
        mItemUnity = RnsStdrspotUnity.FindByStdrspotSn(exStdrspotSn);
        setTitle();
        ...
        adapter = new ArrayAdapter<>(InvestInsertInfo.this, android.R.layout.simple_spinner_dropdown_item, mHeadOfficeInfoList.getCodeValueList());
        ...
        adapter = new ArrayAdapter<>(InvestInsertInfo.this, android.R.layout.simple_spinner_dropdown_item, mPositionInfoList.getCodeValueList());
    ...
    private void setBranchList(boolean isRepresent, String branchCode) {
        ...
        ArrayAdapter adapter = new ArrayAdapter<>(InvestInsertInfo.this, android.R.layout.simple_spinner_dropdown_item, codeRecordList.getCodeValueList());
    ...
    private boolean onSave(String inqSttus) {
        ...
        if (representInqUserName.equals(empty) || representInqUserId.equals(empty)) {
            Toast.makeText(InvestInsertInfo.this, getString(R.string.message_error_empty_name, "대표"), Toast.LENGTH_SHORT).show();
            return false;
        }
        ...
        if (realInqUserName.equals(empty) || realInqUserId.equals(empty)) {
            Toast.makeText(InvestInsertInfo.this, getString(R.string.message_error_empty_name, "실제"), Toast.LENGTH_SHORT).show();
            return false;
        }

        if (null == mItemBf) {
            mItemBf = new RnsSttusInqBefore();
            mItemBf.setSttusBfSn(exStdrspotSn);
            mItemBf.setStdrspotSn(exStdrspotSn);
            mItemBf.setBasePointNo(mItemUnity.getBasePointNo());
            mItemBf.setProgression(mProgression);

            String pointNm = mItemUnity.getPointsNm();
            String pointknd = mItemUnity.getPointsKndCd();
            String jgrcCode = mItemUnity.getJgrcCd();
            String brhCode = mItemUnity.getBrhCd();

            mItemBf.setPointsNm(null == pointNm ? empty : pointNm);
            mItemBf.setPointsKndCd(null == pointknd ? empty : pointknd);
            mItemBf.setJgrcCd(null == jgrcCode ? empty : jgrcCode);
            mItemBf.setBrhCd(null == brhCode ? empty : brhCode);
        }
        ...
        mItemBf.setInqClsf(positionCode);
    ...
    public void onNext() {
        if (onSave("1")) {
            Intent moveIntent = mInsertUtil.getMoveIntent(R.id.imgbtn_mtr_sttus);
            startActivity(moveIntent);
    ...
    void setTitle() {
        if (mItemUnity != null) {
            String pointNm = mItemUnity.getPointsNm() != null ? mItemUnity.getPointsNm() : "";
            String pointknd = mItemUnity.getPointsKndCd() != null ? DBUtil.getCmmnCode("100", mItemUnity.getPointsKndCd()) : "";
    ...
    private Response.Listener<String> successListener = new Response.Listener<String>() {
        ...
        if (null == userInfo) {
            Toast.makeText(InvestInsertInfo.this, getString(R.string.message_invalidate_user_id), Toast.LENGTH_SHORT).show();
        } else {
            //사용자명
            EditText editbox;
            Spinner spinner;
    ...
    private Response.ErrorListener failListener = new Response.ErrorListener() {
        ...
        public void onErrorResponse(VolleyError e) {
            Toast.makeText(InvestInsertInfo.this, getString(R.string.message_error_server_trans), Toast.LENGTH_SHORT).show();
        }
    };
