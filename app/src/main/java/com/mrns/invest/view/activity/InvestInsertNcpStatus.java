...
public class InvestInsertNcpStatus extends AppCompatActivity {
    //! Extras
    ...
    protected void InitView() {
        ...
        setData(RnsNpUnity.FindByNpuSn(exStdrspotSn));
        String arrTrue = "";
        if(null == mItem){
            mItem = new RnsNpSttusInqBf();
            String progre = "0/0/0/0/0/0/0/0";
            String[] array =  progre.split("/", -1);//mItem.getProgression().split("/", -1);
            arrTrue = "";
        ...
        if (mItem != null) {
          ...
          //  String pointNm = mItem.getPointsNm() != null ? mItem.getPointsNm() : getString(R.string.empty);
            ...
            if (mBIsMtrSttus) {
                ...
                ArrayAdapter<String> adapter = new ArrayAdapter<>(InvestInsertNcpStatus.this, android.R.layout.simple_spinner_dropdown_item, arrayMtrCode);
                ...
                if (null == inqMtrCode || inqMtrCode.equals(getString(R.string.empty))) {
                    ...
                    for (int i = 0; i < mMtrCodeRecords.size(); i++) {
                        if (inqMtrCode.equals(mMtrCodeRecords.get(i).getCode())) {
                            spinnerMtrSttus.setSelection(i);
                            break;
                        }
                    }
                }
...
//            arrayRadiobtn.get(mRadioIndex).setChecked(true);
    ...
    private void onSave(String inqSttus) {
        ...
        if (mRadioIndex != -1) {
            if (mBIsMtrSttus) {
                ...
                mItem.setInqMtrCd(mMtrCodeRecords.get(selectedIndex).getCode());
            } else {
    ...
    public void onNext(View view) {
        ...
        if(mBIsMtrSttus){
            Intent intentTest = mInsertUtil.getMoveIntentNcp(R.id.imgbtn_rtk_able);
            startActivity(intentTest);
           // layoutMtrPart.setVisibility(View.GONE);
        }else{
            Intent intentTest = mInsertUtil.getMoveIntentNcp(R.id.imgbtn_etc);
            startActivity(intentTest);
        }
    ...
    public void onBack() {
        if (!mBIsMtrSttus) {
            Intent intentTest = mInsertUtil.getMoveIntentNcp(R.id.imgbtn_mtr_sttus);
            startActivity(intentTest);
            layoutMtrPart.setVisibility(View.GONE);
        } else {
            onBackPressed();
